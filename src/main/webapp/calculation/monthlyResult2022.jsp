...
<div class="container mt-3 mb-5">
	<table>
		...
		<th>수입</th>
			<% 
			String sql = "SELECT SUBSTR(DATETIME, 1, 5) AS MONTH, SUM(AMOUNT) AS TOTAL_INCOME FROM ACCOUNT WHERE TYPE='입금' AND SUBSTR(DATETIME, 1, 2)='22' AND ID=? GROUP BY SUBSTR(DATETIME, 1, 5) ORDER BY SUBSTR(DATETIME, 1, 5) ASC"; 
		...
		<th>지출</th>
			<% 
			sql = "SELECT SUBSTR(DATETIME, 1, 5) AS MONTH, SUBSTR(SUM(AMOUNT), 2) AS TOTAL_EXPENDITURE FROM ACCOUNT  WHERE TYPE='출금' AND SUBSTR(DATETIME, 1, 2)='22' AND ID=? GROUP BY SUBSTR(DATETIME, 1, 5) ORDER BY SUBSTR(DATETIME, 1, 5) ASC";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, ID);
			rs = pstmt.executeQuery();
